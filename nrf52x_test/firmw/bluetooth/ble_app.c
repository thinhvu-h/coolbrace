...
static void battery_level_update(void)
{
    ...
    if (m_bas_notif_enable) {
        ...
        {
            NRF_LOG_INFO(" ... Sending => FAILED");
            APP_ERROR_HANDLER(err_code);
        } else {
            NRF_LOG_INFO(" ... Sending => SUCCESSED");
        }
    }
}
...
static void temperature_update(void)
{
    ...
    NRF_LOG_INFO("Temperature: %d", temperature_meas.temp_in_celcius.mantissa);
    if (!m_hts_meas_ind_conf_pending)
    {
        ...
        switch (err_code)
        {
            ...
            case NRF_ERROR_INVALID_STATE:
                NRF_LOG_INFO(" ... Sending => INVALID_STATE");
                // Ignore error.
                break;
